...
def CreatModel(input_shape = (28, 28, 1), output_shape = 10):
    ...
    x = SeparableConv2D(filters = f, kernel_size = (3, 3), strides = (1, 1), padding = 'same')(x)
    x = BatchNormalization()(x)
    x_shortcut = SeparableConv2D(filters = f, kernel_size = (3, 3), strides = (1, 1), padding = 'same')(input_tensor)
    ...
    x = SeparableConv2D(filters = f * 2, kernel_size = (3, 3), strides = (1, 1), padding = 'same')(x1)
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=0.3)(x)
    x = SeparableConv2D(filters = f * 2, kernel_size = (3, 3), strides = (1, 1), padding = 'same')(x1)
    x = BatchNormalization()(x)
    ...
    x = BatchNormalization()(x)
    x_shortcut = SeparableConv2D(filters = f * 2, kernel_size = (3, 3), strides = (1, 1), padding = 'same')(x1)
    x_shortcut = BatchNormalization()(x_shortcut)
    x = Add()([x, x_shortcut])
    x = LeakyReLU(alpha=0.3)(x)
    x = MaxPool2D(pool_size = (2, 2), strides = (2, 2), padding = 'same')(x)
